...
const searchStart = c.indexOf('x/* Active Counters */}');
const searchEnd = c.indexOf('{/* Full Screen Modal */}');

// Extract the existing product card JSX dynamically
port cardStart = c.indexOf('<motion.div\n                layout\n                key={product.id}');
...
const section1 = `
          <div className="space-y-16">
            {/* VLP Series Section */}
            {filteredProducts.filter(p => !!p.code.match(/^BLR/) && (parseInt(p.code.replace(/[^0-9]/g, '')) <= 78)).length > 0 && (
              <div>
                <div className="flex items-center gap-4 mb-8">
                  <h2 className="text-2xl md:text-3xl font-black text-slate-900">VLP Series</h2>
                  <div className="h-[1px] flex-1 bg-slate-200 mt-2"></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {filteredProducts
                    .filter(p => !!p.code.match(/^BLR/) && (parseInt(p.code.replace(/[^0-9]/g, '')) <= 78))
                    .map((product) => (
                      ` + card + `
                    ))}
                </div>
...
const hwToggleIcon = `className={\`w5 h-5 transition-transform duration-300 ${showHardware ? 'rotate-180' : ''}\`}`;
const hToggle = `onClick={8) => setShowHardware(!showHardware)}`;
